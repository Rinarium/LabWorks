...
public class Controller {
     ...
     * @param packet - third interacting object.
     */
    public Controller(final GUI gui, final Shop shop, final Buyer buyer,
        ...
        buttonPutToPacketAction(gui, buyer, packet);
        buttonGiveOutPacketAction(gui, buyer, packet);
    }
    ...
    private void buttonInventoryAction(final GUI gui) {
        gui.getBtnInventory().setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(final ActionEvent e) {
                ...
                gui.getPacketBox().setVisible(!check);
        ...
        gui.getBtnTakeProduct().setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(final ActionEvent event) {
                ...
                if (transferProduct(shop, buyer, index)) {
                    if (index == Constants.BREAD_ID) {
                    ...
                    } else if (index == Constants.MATCH_ID) {
                        ...
                        gui.updateData(gui.getQuantityInfoLabel(),
                                milk.getQuantity());
                        gui.getHandStatus().setText("молоко");
                    }
                }
            }
        });
     ...
     * This method sets the action when the button is clicked.
     * @param gui - interface which contains button.
     * @param shop to get access to cart.
        ...
        gui.getBtnLayOut().setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(final ActionEvent event) {
                ...
                for (Product product : shop.getCart().getProduct()) {
                    ...
                    } else if (product.getID() == Constants.MATCH_ID) {
                        Match.incrementQuantity();
                    } else {
                        Milk.incrementQuantity();
                    }
        ...
        gui.getMainProductCombo().setOnHiding(new EventHandler<Event>() {
            ...
            public void handle(final Event event) {
                ...
                if (id == Constants.BREAD_ID) {
                    ...
                    gui.updateData(gui.getFeatureStatus(),
                            Constants.MILK_AMOUNT);
                    gui.getFeatureNameStatus().setText("Литров: ");
                    gui.getBtnConsume().setText("Выпить");
                }
            }
        });
     ...
     * @param gui - interface which contains button.
     * @param buyer - buyer is source of product.
     * @param packet - packet is receiver of products.
     */
    private void buttonPutToPacketAction(final GUI gui, final Buyer buyer,
                                   final Packet packet) {
        gui.getBtnPutToPacket().setOnAction(new EventHandler<ActionEvent>() {
            @Override
            public void handle(final ActionEvent e) {
                boolean check = buyer.putToPacket(packet);

                if (check) {
                    gui.updateData(gui.getPacketQuantityStatus(),
                            packet.getProducts().size());
                    gui.updateData(gui.getMainProductStatus(), 0);
                } else {
                    GUI.warning("Нет купленных продуктов!");
                }
            }
        });
    }
     ...
     * This method transfer product from buyer to packet.
     * @param gui - interface which contains button.
     * @param buyer - buyer is receiver of products.
     * @param packet - packet is source of product.
     */
    private void buttonGiveOutPacketAction(final GUI gui, final Buyer buyer,
                                         final Packet packet) {
        gui.getBtnGiveOutPacket().setOnAction(new EventHandler<ActionEvent>() {
            @Override
            public void handle(final ActionEvent e) {
                boolean check = buyer.receiveFromPacket(packet);
